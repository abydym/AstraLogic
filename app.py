...
with tab3:
    ...
    st.markdown("与 Agent 对话，让 AI 帮你分析和优化通信链路")
    ...
    if "messages" not in st.session_state:
        st.session_state.messages = [
            {
                "role": "assistant", 
                "content": "你好！我是 AstraLogic 卫星通信仿真 Agent。\n\n"
                          "我可以帮你：\n🔹 分析当前仿真状态\n🔹 优化通信链路参数\n🔹 诊断通信问题\n🔹 预测卫星可见窗口\n\n"
                          f"当前仿真时间: {current_time:.1f} 秒\n"
                          f"活跃平台: {', '.join([f'平台{p}' for p in selected_platforms])}\n\n"
                          "请告诉我你需要什么帮助？"
            }
        ]
    ...
    if prompt := st.chat_input("输入你的问题..."):
        ...
        with st.chat_message("assistant"):
            with st.spinner("Agent 正在分析..."):
                # 这里可以调用真实的Agent
                response = f"基于当前仿真状态（时间: {current_time:.1f}秒）：\n\n"
                
                # 添加一些分析
                if "距离" in prompt or "链路" in prompt:
                    if df_distance is not None:
                        current_dist = df_distance[df_distance['simTime'] == current_time]['distance'].values
                        if len(current_dist) > 0:
                            dist = current_dist[0]
                            response += f"• 当前卫星间距离: {dist:.1f} km\n"
                            response += f"• 通信状态: {'正常' if dist <= 10000 else '可能中断'}\n"
                            response += f"• 建议: {'保持当前配置' if dist <= 10000 else '考虑增加发射功率或切换到其他链路'}\n"
                    ...
                    for platform in selected_platforms:
                        ...
                        if len(p_data) > 0:
                            velocity = p_data.iloc[0]['velocity_magnitude']
                            response += f"• 平台 {platform} 速度: {velocity:.2f} km/s\n"
                
                elif "状态" in prompt:
                    response += f"• 仿真状态: {'运行中' if st.session_state.running else '已停止'}\n"
                    response += f"• 当前时间: {current_time:.1f} / {df_entity['simTime'].max():.1f} 秒\n"
                    response += f"• 数据点数: {len(df_entity)}\n"
                
                else:
                    response += "我理解你的问题。基于当前仿真数据，我可以提供以下建议：\n"
                    response += "1. 检查卫星间距离是否在通信范围内\n"
                    response += "2. 验证卫星速度是否符合轨道力学\n"
                    response += "3. 分析轨道参数是否稳定\n\n"
                    response += "请提供更具体的问题，我可以给出更详细的分析。"
